 ...
 * @version 1.0
 */
public class GridLayerCollector extends
        DefaultLayerCollector<GridDimension, GridCompositeLayer, GridRecord> {

    public GridLayerCollector(ILayerStore store) {
    ...
    protected void addToDims(GridCompositeLayer layer, GridRecord rec) {
        ...
        for (SimpleDimension d : dims) {
            ...
            if (GridDimension.REFTIME_DIM.equals(name)) {
                values.add(LayerTransformer.format(dt.getRefTime()));
            } else if (GridDimension.FORECAST_OFFSET_DIM.equals(name)) {
            ...
            } else if (GridDimension.PARAM_DIM.equals(name)) {
    ...
    private String getParameter(GridRecord rec) {
        ...
        return GridRecordFinder.dbToOgcParameter(fieldName);
    }
    ...
    protected boolean initializeLayer(GridCompositeLayer layer, GridRecord rec) {
        ...
        TreeSet<GridDimension> dims = new TreeSet<GridDimension>();
        dims.add(new GridDimension(GridDimension.REFTIME_DIM, "ISO8601"));
        dims.add(new GridDimension(GridDimension.FORECAST_OFFSET_DIM, "ISO8601"));
        String levelName = LevelDimUtil.LEVEL_DIM_PREFIX + master.getName();
        dims.add(new GridDimension(levelName, levelUnit));
        dims.add(new GridDimension(GridDimension.PARAM_DIM, parameter.getUnit()
                .toString()));
        layer.addDimensions(getParameter(rec), dims);
        return true;
    }

    protected ReferencedEnvelope getProperBounds(GridLayer layer,
