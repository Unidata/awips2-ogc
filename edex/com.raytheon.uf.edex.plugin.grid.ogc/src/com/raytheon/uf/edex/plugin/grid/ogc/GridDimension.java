...
/**
 * OGC dimension for gridded data
 * 
 * <pre>
 * 
 * SOFTWARE HISTORY
 ...
 * ------------ ---------- ----------- --------------------------
 * Nov 13, 2015            bclement     Initial creation
 * 
 * </pre>
 * 
 * @author bclement
...
@DynamicSerialize
public class GridDimension extends SimpleDimension {
    ...
    protected transient DataTime defaultTime = null;
    ...
    public GridDimension(String name, String units) {
        this.name = name;
        this.units = units;
     ...
     * @param otherDim
     */
    public GridDimension(GridDimension other) {
        super(other);
    ...
    protected DataTime getDefaultTime(SimpleLayer<?> layer) {
        if (defaultTime == null) {
            try {
                SortedSet<DataTime> times = ForecastTimeUtil.getDataTimes(
                        layer, layer.getDefaultTime(),
     ...
     * @param from
     */
    public static void copy(Collection<GridDimension> to,
            Collection<GridDimension> from) {
        for (GridDimension d : from) {
            to.add(new GridDimension(d));
        }
    }
