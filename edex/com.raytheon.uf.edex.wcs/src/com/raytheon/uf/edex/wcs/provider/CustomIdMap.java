...
public class CustomIdMap {
    ...
    private static LocalizationFile findConfigFile() {
        ...
        for (LocalizationContext ctx : searchHierarchy) {
            LocalizationFile localizationFile = pm
                    .getLocalizationFile(ctx, "wcs" + IPathManager.SEPARATOR
    ...
    private static Map<String, String> loadConfig(LocalizationFile idMapFile) {
        ...
        try (InputStream in = idMapFile.openInputStream()) {
            props.load(in);
        } catch (Exception e) {
            log.error("Unable to load WCS id map: " + idMapFile, e);
